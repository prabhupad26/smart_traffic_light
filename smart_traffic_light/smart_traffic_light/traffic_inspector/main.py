...
from smart_traffic_light.traffic_inspector.helper import initialize_vision_objects
from smart_traffic_light.traffic_inspector.helper import draw_boxes, save_with_bbox_renders
...
class TrafficInspector:
    ...
    def run_simulation(self):
        ...
        while sim_counter <= self.ttl_simulation:
            ...
            for stl in self.stl_list:
                ...
                if stl['status'] == 'green':
                    ...
                    self.active_count = sum(traffic_details.values())
                    ...
                    if skip_scene:
                        ...
                        self.inactive_count = sum(stl['inactive_lane_glimpse'].values())
        ...
        # closing video files
        [stl['close_vids']() for stl in self.stl_list]
    ...
    def process_image_mode(self):
        ...
        for img in [lane1_img, lane2_img]:
            ...
            for prediction in response.json():
                ...
                if prediction['name'] in self.priority_v_labels:
                    result_dict[img]['priority_vehicle_count'] += 1
                box = (prediction['xmin'], prediction['ymin'],
                       prediction['xmax'], prediction['ymax'])

                newly_rendered_image = draw_boxes(box, prediction['xmin'],
                                                  prediction['ymin'], newly_rendered_image,
                                                  prediction['name'])
    ...
    def scheduled_version(self):
        """
        To show the without AI version
        """
        stl_timer_default = self.stl_timer_default
    ...
    def get_traffic_count(self, iter_=None, skip_scene=0):
        if skip_scene > 0 and iter_:
            ...
            return sum(traffic_details.values())

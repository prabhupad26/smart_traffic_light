...
class Lane:
    def __init__(self, number: int, status: AnyStr = None, direction: AnyStr = None,
                 exit_lane: List = None, video_file: AnyStr = None) -> None:
        self.number = number
        self.status = status
    ...
    def get_frame(self, width: int = 640, height: int = 640) -> ByteString | None:
        ...
        while self.video.isOpened():
            ...
            print(f"Sending video feed {sys.getsizeof(buffer.tobytes())} bytes ------> Flask inference api")
    ...
    def get_element_count(self) -> int:
        for frame in self.get_frame():
            ...
            yield len(response_json), self.number
...
if __name__ == '__main__':
    input_data_path = "data\\videos\\scenario_1"
